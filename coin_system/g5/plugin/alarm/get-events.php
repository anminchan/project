...
if ($act =='alarm') {
    ...
    if($gubun == 'admin'){
        ...
        $row = sql_fetch(" select * from {$g5['coin_req_table']} where cr_state = 6 and cr_date >= NOW() - INTERVAL 1 HOUR order by cr_id desc limit 1 ");

        if ($row) {
            $result['content'] = $row['mb_name'].' 님께서 '.$row['cr_coin'].' Coin 전환을 신청하셨습니다.';
            $result['msg'] = 'SUCCESS';
            $result['cr_id'] = $row['cr_id'];
            //$result['sound'] = 'N';
            $result['title'] = $row['mb_name'];
            $result['url'] = G5_ADMIN_URL . '/coin_changelist.php?cr_state=6';
        }else{
            $result['msg'] = 'NOMSG';
            $result['cr_id'] = '';
        }
